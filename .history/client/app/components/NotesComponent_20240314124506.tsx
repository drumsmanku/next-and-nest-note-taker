  ...
  return (
    <div className='flex flex-col h-screen w-screen items-center'>
      ...
      <div className='flex flex-col w-[50%]'>
        {showCreatePopup && (
          <div className='flex flex-col w-[80%] border-2 rounded-lg  bg-yellow-400 px-20'>
            <input
              type="text"
              placeholder="Title"
              value={newNoteTitle}
        ...
        <ul className='flex'>
          {notes.map((note) => (
            <li className='border-2 p-10' key={note._id}>
              <div>
                <strong>{note.title}</strong>
                <p>{note.description}</p>
              </div>
              <button className="px-4 py-1.5 text-white h-8 text-md rounded-full border-none bg-[#36416A] mt-8" onClick={() => {
