...
const NotesComponent: React.FC = () => {
  ...
  const fetchNotes = async () => {
    try {
      const response = await axios.get<Note[]>('https://next-and-nest-note-taker-2.onrender.com/notes');
  ...
  const handleAddNote = async () => {
    try {
      await axios.post('https://next-and-nest-note-taker-2.onrender.com/notes', {
        title: newNoteTitle,
  ...
  const handleEditNote = async () => {
    try {
      await axios.put(`https://next-and-nest-note-taker-2.onrender.com/notes/${editNoteId}`, {
        title: editNoteTitle,
  ...
  const handleDeleteNote = async (noteId: string) => {
    try {
      await axios.delete(`https://next-and-nest-note-taker-2.onrender.com/notes/${noteId}`);
      fetchNotes();
    } catch (error) {
  ...
  return (
    <div className='flex flex-col h-screen w-screen items-center'>
      <button className="px-4 py-1.5 mb-5 text-white h-8 text-md rounded-full border-none bg-[#36416A] mt-8" onClick={() => setShowCreatePopup(true)}>Add Note</button>
